...
| Phase -0.5 — operational controls | PASS | PR template + release-channel + incident protocol artifacts are now present. | `.github/PULL_REQUEST_TEMPLATE.md`, `docs/RELEASE_CHANNELS.md`, `docs/RENDERER_INCIDENT_PROTOCOL.md` |
...
| Phase 2 — camera/input parity | PASS | Phase-2 parity script passes with focal drift within threshold. | `tests/baseline/run_phase2_interaction_parity.js` (`[PHASE2_PARITY] PASS`) |
...
| Phase 4 — media visual port | PASS | Media delegates + DTO bridge + text edit commit path implemented, and perf delta gate passes on captured logs. | `resources/qml/ImageItem.qml`, `resources/qml/VideoItem.qml`, `resources/qml/TextItem.qml`, `resources/qml/CanvasRoot.qml`, `src/frontend/rendering/canvas/QuickCanvasController.cpp`, `tests/baseline/run_phase4_visual_parity.js`, `tests/baseline/run_phase4_perf_delta.js`, `/tmp/mouffette_phase4_perf_quick_auto_targeted.log`, `/tmp/mouffette_phase4_perf_legacy_auto.log` |
