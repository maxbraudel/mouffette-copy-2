...
void ScreenCanvas::mousePressEvent(QMouseEvent* event) {
    ...
    if (event->button() == Qt::LeftButton) {
        ...
        if (m_scene) {
            ...
            for (QGraphicsItem* it : selEarly) if (auto* v = dynamic_cast<ResizableVideoItem*>(it)) { if (v->handleControlsPressAtItemPos(v->mapFromScene(scenePosEarly))) { syncVolumeSettingsPanel(); m_overlayMouseDown = true; event->accept(); return; } }
        }
    ...
    const QList<QGraphicsItem*> hitItems = items(event->pos());
        ...
        if (mediaHit) {
            ...
            if (auto* v = dynamic_cast<ResizableVideoItem*>(mediaHit)) {
                ...
                if (v->handleControlsPressAtItemPos(itemPos)) { syncVolumeSettingsPanel(); event->accept(); return; }
            }
    ...
    for (QGraphicsItem* it : scene()->selectedItems()) if (auto* v = dynamic_cast<ResizableVideoItem*>(it)) { const QPointF itemPos = v->mapFromScene(mapToScene(event->pos())); if (v->handleControlsPressAtItemPos(itemPos)) { syncVolumeSettingsPanel(); event->accept(); return; } }
    if (m_scene) m_scene->clearSelection();
...
void ScreenCanvas::mouseDoubleClickEvent(QMouseEvent* event) {
    ...
    if (event->button() == Qt::LeftButton) {
        ...
        if (m_scene) {
            ...
            const QList<QGraphicsItem*> sel = m_scene->selectedItems();
            for (QGraphicsItem* it : sel) if (auto* v = dynamic_cast<ResizableVideoItem*>(it)) if (v->handleControlsPressAtItemPos(v->mapFromScene(scenePosSel))) { syncVolumeSettingsPanel(); m_overlayMouseDown = true; event->accept(); return; }
...
void ScreenCanvas::mouseMoveEvent(QMouseEvent* event) {
    ...
    if (m_scene) { const QList<QGraphicsItem*> sel = m_scene->selectedItems(); for (QGraphicsItem* it : sel) if (auto* v = dynamic_cast<ResizableVideoItem*>(it)) { if (v->isDraggingProgress() || v->isDraggingVolume()) { v->updateDragWithScenePos(mapToScene(event->pos())); if (v->isDraggingVolume()) { syncVolumeSettingsPanel(); } event->accept(); return; } } }
        event->accept(); return;
    }
    ...
    for (QGraphicsItem* it : sel) if (auto* v = dynamic_cast<ResizableVideoItem*>(it)) if (v->isSelected() && (v->isDraggingProgress() || v->isDraggingVolume())) { v->updateDragWithScenePos(mapToScene(event->pos())); if (v->isDraggingVolume()) { syncVolumeSettingsPanel(); } event->accept(); return; }
...
void ScreenCanvas::mouseReleaseEvent(QMouseEvent* event) {
    ...
    if (event->button() == Qt::LeftButton) {
        ...
        if (m_overlayMouseDown) {
            if (m_scene) { const QList<QGraphicsItem*> sel = m_scene->selectedItems(); for (QGraphicsItem* it : sel) if (auto* v = dynamic_cast<ResizableVideoItem*>(it)) { if (v->isDraggingProgress() || v->isDraggingVolume()) { v->endDrag(); syncVolumeSettingsPanel(); } } }
            m_overlayMouseDown = false; event->accept(); return;
        }
    for (QGraphicsItem* it : m_scene->items()) if (auto* v = dynamic_cast<ResizableVideoItem*>(it)) if (v->isSelected() && (v->isDraggingProgress() || v->isDraggingVolume())) { v->endDrag(); syncVolumeSettingsPanel(); event->accept(); return; }
...
void ScreenCanvas::syncVolumeSettingsPanel() {
    if (!m_globalSettingsPanel) return;
    m_globalSettingsPanel->syncVolumeFromMedia();
}
